...
factor_model_type = st.sidebar.selectbox(
    "Factor model",
    ["PCA", "Single Market Factor (SPY)", "Sector ETF factors (auto mapping)"]
)
...
if mode == "Backtest":
    ...
    leverage_long = st.sidebar.number_input(
        "Long leverage (× equity)", 0.0, 5.0, 2.0, 0.5
    )
    leverage_short = st.sidebar.number_input(
        "Short leverage (× equity)", 0.0, 5.0, 2.0, 0.5
    )
    ...
    # Use persistent session state flag
    ...
    if st.sidebar.button("Compute Live Signals"):
        st.session_state["run_backtest"] = True  # reuse flag for live mode as well
...
def build_residuals_pca(
    ...
    """
    Build PCA residuals using robust PCA.
    Uses returns over the pca_lookback window for the **whole universe**.
    Then extracts each ticker's residual series.
    """
    # Use the last pca_lookback days for all tickers together
    price_window = prices[tickers].iloc[-pca_lookback:]
...
def fit_ou_from_residuals(
    ...
    good = np.isfinite(x_tm1) & np.isfinite(x_t)
    x_tm1 = x_tm1[good]
    x_t = x_t[good]
...
def build_residuals(
    ...
    if factor_model_type == "PCA":
        # PCA lookback uses your window_corr_days parameter
        residuals_df = build_residuals_pca(
            prices,
            tickers=user_tickers,
            pca_lookback=window_corr_days
        )
        # convert DataFrame -> dict[ticker] = Series
        residuals = {
        ...
        if "SPY" not in returns.columns:
            raise ValueError("SPY returns not available – ensure SPY is in ticker list.")
        return build_residuals_single_factor_rolling(
            returns,
            market_ticker="SPY",
            beta_window=beta_window
        )
    else:  # sector ETF rolling
...
def generate_signals_and_backtest(
    ...
    for i, date in enumerate(all_dates):
        ...
        # 6) Apply transaction costs (portfolio level)
...
# =========================================================
# HELPER: PER-STOCK P&L RECONSTRUCTION
# =========================================================

def compute_per_stock_pnl(
    equity_df: pd.DataFrame,
    detailed_df: pd.DataFrame,
    prices: pd.DataFrame
) -> pd.Series:
    """
    Reconstructs per-stock gross P&L by:
      PnL_ticker(d) = qty_{d-1} * (price_d - price_{d-1})
    using position snapshots from detailed_df.
    This is *gross* P&L (before transaction costs allocation).
    """
    if equity_df.empty or detailed_df.empty:
    ...
    for i in range(1, len(dates)):
        ...
        # positions at end of previous day
        try:
            pos_prev = detailed_df.xs(d_prev, level="date")["qty"]
        except KeyError:
            continue

        for ticker in tickers:
            qty_prev = pos_prev.get(ticker, 0.0)
            if qty_prev == 0.0:
                continue
            if ticker not in prices.columns:
                continue
            # guard against missing price
            if d_prev not in prices.index or d not in prices.index:
                continue
            p_prev = prices.loc[d_prev, ticker]
            p_curr = prices.loc[d, ticker]
            if not (np.isfinite(p_prev) and np.isfinite(p_curr)):
                continue
            pnl = qty_prev * (p_curr - p_prev)
            pnl_by_ticker[ticker] += float(pnl)

    pnl_series = pd.Series(pnl_by_ticker)
    # Drop exact zeros to declutter
    pnl_series = pnl_series[pnl_series != 0.0]
    return pnl_series.sort_values(ascending=False)
...
if st.session_state["run_backtest"]:
    if len(user_tickers) < 2:
        ...
        if prices.empty:
            ...
            if factor_model_type == "Sector ETF factors (auto mapping)":
                # st.subheader("Automatic Stock → Sector ETF Mapping")
            ...
            if mode == "Backtest":
                ...
                if equity_df.empty:
                    ...
                    init_eq = equity_df["equity"].iloc[0]
                    final_eq = equity_df["equity"].iloc[-1]
                    total_return = final_eq / init_eq - 1
                    ...
                    # Win rate = % of positive PnL days
                    win_days = (daily_ret > 0).sum()
                    num_days = len(daily_ret)
                    win_rate = win_days / num_days * 100 if num_days > 0 else 0.0

                    num_trades = len(trades_df) if not trades_df.empty else 0

                    k1, k2, k3, k4, k5 = st.columns(5)
                    k1.metric("Total Return", f"{total_return*100:.1f}%")
                    k2.metric("Sharpe Ratio", f"{sharpe:.2f}")
                    k3.metric("Max Drawdown", f"{max_dd*100:.1f}%")
                    k4.metric("Win Days %", f"{win_rate:.1f}%")
                    k5.metric("# Trades", f"{num_trades}")

                    # =====================================================
                    # EQUITY CURVE & EXPOSURE
                    # =====================================================
                    st.subheader("Equity Curve & Exposure")
                    ...
                    # =====================================================
                    st.subheader("📈 Per-Stock Return Attribution")

                    stock_pnl = compute_per_stock_pnl(equity_df, detailed_df, prices)
                    if stock_pnl.empty:
                        st.info("No per-stock P&L could be computed (likely no active positions).")
                    else:
                        stock_ret_pct = stock_pnl / init_eq * 100.0
                        stock_ret_df = pd.DataFrame({
                            "Return % of Initial Equity": stock_ret_pct
                        }).sort_values("Return % of Initial Equity", ascending=False)

                        c1, c2 = st.columns([2, 1])
                        with c1:
                            fig_pnl = px.bar(
                                stock_ret_pct,
                                x=stock_ret_pct.index,
                                y=stock_ret_pct.values,
                                labels={"x": "Ticker", "y": "Return % of Initial Equity"},
                                title="Per-Stock Return Contribution"
                            )
                            st.plotly_chart(fig_pnl, use_container_width=True)

                        with c2:
                            st.dataframe(stock_ret_df.style.format({"Return % of Initial Equity": "{:.2f}%"}))

                    # =====================================================
                    # PER-STOCK LAST-DAY S-SCORES
                    # =====================================================
                    ...
                    # =====================================================
                    # PER-TICKER TRADE LOG & SIGNAL CHARTS
                    # =====================================================
                    if not trades_df.empty:
                        ...
                        # Mini dashboard for this ticker
                        st.markdown(f"### 📌 {selected_ticker} – Ticker Dashboard")

                        if not stock_pnl.empty:
                            ticker_pnl = float(stock_pnl.get(selected_ticker, 0.0))
                            ticker_ret_pct = ticker_pnl / init_eq * 100.0
                            total_pnl = stock_pnl.sum()
                            contrib_pct = (ticker_pnl / total_pnl * 100.0) if total_pnl != 0 else 0.0
                        else:
                            ticker_pnl = 0.0
                            ticker_ret_pct = 0.0
                            contrib_pct = 0.0
                        ...
                        d1.metric("Gross P&L ($)", f"{ticker_pnl:,.2f}")
                        d2.metric("Return % of Equity", f"{ticker_ret_pct:.2f}%")
                        d3.metric("Share of Strategy P&L", f"{contrib_pct:.1f}%")
                        ...
                        # ETF for overlay (sector ETF or SPY)
                        selected_etf = None
                        if factor_model_type == "Sector ETF factors (auto mapping)":
    ...
    st.info("Configure parameters in the sidebar and click **Run Backtest** or **Compute Live Signals** to start.")
